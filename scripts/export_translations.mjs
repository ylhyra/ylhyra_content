...
const missingWords = [];
const audioAlignment = checkAudioAlignments(ROOT);
...
say("## Forced audio alignment\n");
say(
  `${audioAlignment.audioFiles} audio files, ${audioAlignment.sections} timed sections, ` +
    `${audioAlignment.alignedIDs} aligned ID references.\n`
);
if (audioAlignment.errors.length) {
  say(`### Structural errors (${audioAlignment.errors.length})\n`);
  for (const error of audioAlignment.errors) say(`- ${error}`);
  say("");
} else {
  say("No missing IDs, invalid timing ranges, stale XML hashes, or mismatches between the");
  say("runtime synchronization list and the original forced-alignment IDs.\n");
}
if (audioAlignment.textDrift.length) {
  say(`### Aligned sentence text changed since synchronization (${audioAlignment.textDrift.length})\n`);
  say("The IDs and timing remain usable, but the stored Aeneas transcript differs from the");
  say("current sentence after punctuation, spelling, or wording edits. Review substantial changes.\n");
  for (const drift of audioAlignment.textDrift) say(`- ${drift}`);
  say("");
}
...
console.log(
  ...
  `audio alignment: ${audioAlignment.errors.length} error(s), ` +
  `${audioAlignment.textDrift.length} text drift(s)\n` +
  `wrote exports/ (review/, *.tsv, translations.json, checks.md)`
);

if (audioAlignment.errors.length) process.exitCode = 1;
